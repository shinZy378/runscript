...
class menu:
	...
	def main(self):
		...
		print(' %s║'%(N))
		print(' %s╠═[%s0%s] %scrack dari daftar teman%s%s'%(N,O,N))
		print(' %s╠═[%s1%s] %scrack dari akun publik%s%s'%(N,O,N))
		print(' %s╠═[%s2%s] %scrack dari akun massal%s%s'%(N,O,N))
		print(' %s╠═[%s3%s] %scrack dari postingan%s%s'%(N,O,N))
		print(' %s╠═[%s4%s] %scrack dari likes post%s%s'%(N,O,N))
		print(' %s╠═[%s5%s] %scrack dari followers%s%s'%(N,O,N))
		print(' %s╠═[%s6%s] %scek opsi akun chekpoint%s%s'%(N,O,N))
		print(' %s╠═[%s7%s] %scek hasil crack ok,cp%s%s'%(N,O,N))
		print(' %s╠═[%s8%s] %sseting User-Agent%s%s'%(N,O,N))
		print(' %s╠═[%s9%s] %scrack email%s%s'%(N,O,N))
		print(' %s╠═[%sG%s] %sGet data² facebook%s%s'%(N,O,N))
		print(' %s╠═[%sK%s] %sLapor bug script%s%s'%(N,O,N))
		print(' %s╠═[%sA%s] %sKeluar, hapus token%s%s'%(N,O,N))
		print(' %s╠═[%sU%s] %sUpgrade ke premium%s%s'%(N,O,N))
		self.pilih()
